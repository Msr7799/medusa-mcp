...
export const env = {
    ...
    medusaAuthType: (process.env.MEDUSA_AUTH_TYPE || process.env.MEDUSA_AUTH_MODE || "basic").toLowerCase(),
    // Backward compatible env var names:
...
export function assertEnv() {
    const missing = [];
    if (!env.medusaBaseUrl)
        missing.push("MEDUSA_BASE_URL");
    if (!env.medusaAuthToken)
        missing.push("MEDUSA_SECRET_API_KEY (or MEDUSA_AUTH_TOKEN)");
    if (env.requireConfirm && !env.confirmSecret)
        missing.push("CONFIRM_SECRET");
    if (missing.length) {
