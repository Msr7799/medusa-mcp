...
export function signConfirmToken(payload) {
...
export function verifyConfirmToken(token) {
    ...
    if (parts.length !== 2)
        throw new Error("Invalid confirm token format");
    ...
    if (payload.iat > now + 30)
        throw new Error("Confirm token from the future");
    return payload;
}
export function mintConfirmToken(req) {
    const now = Math.floor(Date.now() / 1000);
    const payload = {
        v: 1,
        iat: now,
